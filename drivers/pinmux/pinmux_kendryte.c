 ...
 *
 */
typedef struct _kendryte_fpioa_assign_t
{
    uint32_t ch_sel : 8;
...
} __attribute__((packed, aligned(4))) kendryte_fpioa_assign_t;

/* Function list */
static const kendryte_fpioa_assign_t function_config[FUNC_MAX] =
{
    {
...
static inline int pinmux_kendryte_pullup(struct device *dev, u32_t pin, u8_t func)
{
	return 0;
}

static inline int pinmux_kendryte_input(struct device *dev, u32_t pin, u8_t func)
{
	return 0;
}

static int fpioa_set_function_raw(volatile kendryte_fpioa_t *fpioa, int pin,
			   fpioa_function_t function)
{
	/* Check parameters */
	if (pin < 0 || pin >= FPIOA_NUM_IO || function < 0 || function >= FUNC_MAX)
		return -1;
...
static int pinmux_kendryte_set(struct device *dev, u32_t pin, u32_t function)
{
	volatile kendryte_fpioa_t *fpioa = DEV_PINMUX(dev);
	uint8_t index = 0;
	...
	if (function == FUNC_RESV0) {
		fpioa_set_function_raw(fpioa, pin, FUNC_RESV0);
		return 0;
	}

	/* Compare all IO */
	for (index = 0; index < FPIOA_NUM_IO; index++) {
		if ((fpioa->io[index].ch_sel == function) && (index != pin))
			fpioa_set_function_raw(fpioa, index, FUNC_RESV0);
	}

	fpioa_set_function_raw(fpioa, pin, function);

	return 0;
}

static int pinmux_kendryte_get(struct device *dev, u32_t pin, u32_t *func)
{
	volatile kendryte_fpioa_t *fpioa = DEV_PINMUX(dev);

	*func = fpioa->io[pin].ch_sel;

	return -EINVAL;
}

static int pinmux_kendryte_init(struct device *dev)
{
	volatile kendryte_fpioa_t *fpioa = DEV_PINMUX(dev);
	int i = 0;
...
static const struct pinmux_driver_api pinmux_kendryte_driver_api = {
	.set = pinmux_kendryte_set,
	.get = pinmux_kendryte_get,
	.pullup = pinmux_kendryte_pullup,
	.input = pinmux_kendryte_input,
};
...
DEVICE_AND_API_INIT(pinmux_kendryte, CONFIG_KENDRYTE_PINMUX_NAME,
		    &pinmux_kendryte_init, NULL,
		    &pinmux_kendryte_0_config,
		    PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,
		    &pinmux_kendryte_driver_api);
